...
|  | 学習画像にない正常な見た目も、異常として誤検知しうる |

**正常データの範囲に注意**：PatchCore が覚えるのは、学習時に見せた正常品のパッチ特徴です。たとえば照明、材料、製品の種類が変わり、良品でも特徴がメモリバンクから遠くなると、異常スコアが高くなります。反対に、未学習の見た目でも保存済みの特徴に近ければ、異常とは判定されません。つまり「学習していないものは必ず異常」ではなく、**覚えた正常特徴からどれだけ離れているか**で決まります。運用では、良品に生じる自然なばらつきを学習データに含め、条件が変わったらデータと判定閾値を見直します。

**背景のばらつきも重要**：PatchCore は対象物だけでなく、画像に写った背景のパッチも照合します。背景の種類や撮影位置が一定なら、正常な背景の特徴を少数のパターンで覚えやすくなります。一方、検査時だけ別の背景が写ると、製品が良品でも背景部分の距離が大きくなり、画像全体の異常スコアを押し上げることがあります。背景をそろえて撮影する、対象物を切り出す、背景をマスクするなど、**検査に不要な背景のばらつきを減らす**ことが有効です。
